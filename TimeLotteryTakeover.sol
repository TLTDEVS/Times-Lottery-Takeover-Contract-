...
contract TimeLotteryTakeover is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Time Lottery Takeover", "TLT") {
    ...
    function swapBack() private {
        ...
        swapTokensForEth(contractBalance.sub(liquidityTokens)); 
        ...
        uint256 ethForLottery = ethBalance.mul(tokensForLottery1).div(totalTokensToSwap);
        (success,) = address(lotteryAddress1).call{value: ethForLottery}("");
        ethForLiquidity -= ethForLottery;

        ethForLottery = ethBalance.mul(tokensForLottery2).div(totalTokensToSwap);
        (success,) = address(lotteryAddress2).call{value: ethForLottery}("");
        ethForLiquidity -= ethForLottery;
        
        ethForLottery = ethBalance.mul(tokensForLottery3).div(totalTokensToSwap);
